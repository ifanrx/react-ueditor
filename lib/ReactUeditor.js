...
function (_React$Component) {
  ...
  function ReactUeditor(props) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "registerSimpleInsertCode", function () {
      ...
      window.UE.registerUI(name, function (ueditor, uiName) {
        ...
        if (config.render) {
          _this4.setState(function (prevState) {
            return {
              pluginsWithCustomRender: _toConsumableArray(prevState.pluginsWithCustomRender).concat([_objectSpread({
                name: name
              }, config)])
            };
          });
